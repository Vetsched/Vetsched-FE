...
export class UserService {
  ...
  Membersquery(config:any): Observable<{members: [], membersCount: number}> {
    ...
    return this.apiService.get('/members', new HttpParams({ fromObject: params })
    );
  }
  ...
  GroupUsersquery(config:any): Observable<{users: []}> {
    // Convert any filters over to Angular's URLSearchParams
    const params:any = {};
    Object.keys(config.filters1)
    .forEach((key) => {
      params[key] = config.filters1[key];
    });

    return this.apiService.get('/forum_chat_group/finduser', new HttpParams({ fromObject: params })
    );
  }
